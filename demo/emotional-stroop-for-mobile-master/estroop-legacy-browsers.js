...
let expInfo = {'participant': '', 'session': '', 'id': '', 'group': ''};
...
async function experimentInit() {
  ...
  start_button = new visual.TextStim({
    ...
    font: 'Arial',
    units: undefined, 
    pos: [0, (- 0.45)], height: 0.08,  wrapWidth: undefined, ori: 0,
    color: new util.Color('white'),  opacity: 1,
    depth: -4.0 
  });
  ...
  button1 = new visual.Polygon ({
    ...
    ori: 0, pos: [(- buttonGap), ((- buttonOffset) + buttonGap)],
    lineWidth: 5, lineColor: new util.Color([1, 1, 1]),
...
function outer_loopLoopBegin(outer_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    outer_loop.forEach(function() {
      ...
      outer_loopLoopScheduler.add(preBlockRoutineEnd());
      ...
      outer_loopLoopScheduler.add(postBlockRoutineEnd());
      outer_loopLoopScheduler.add(endLoopIteration(outer_loopLoopScheduler, snapshot));
    });
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    trials.forEach(function() {
      ...
      trialsLoopScheduler.add(stroopRoutineEnd());
